...
class Grapher:
    ...
    """
class Grapher:
    def __init__(self, filename, data_fd, image_path=None): # Thêm tham số image_path
        """
        Hàm khởi tạo đã được nâng cấp để xử lý cả ảnh từ tập test và ảnh upload mới.
        """
        self.filename = filename
        self.data_fd = data_fd

        # 1. Đọc file box (dữ liệu OCR)
        box_path = os.path.join(self.data_fd, "box", filename + '.csv')
        try:
            with open(box_path, 'r', encoding='utf-8') as f:
                lines = f.readlines()
            self.df = pd.DataFrame(lines, columns=[0])
        except FileNotFoundError:
            raise FileNotFoundError(f"LỖI: Không tìm thấy file box OCR tại: {box_path}")

        # 2. Đọc file ảnh (Logic đã được nâng cấp)
        # Nếu đường dẫn ảnh được cung cấp trực tiếp (dành cho ảnh upload)
        if image_path and os.path.exists(image_path):
            self.image = cv2.imread(image_path)
        # Nếu không, tìm trong thư mục mặc định (dành cho test theo index)
        else:
            default_image_path = os.path.join(self.data_fd, "img", filename + '.jpg')
            self.image = cv2.imread(default_image_path)
        
        # Kiểm tra xem ảnh có được đọc thành công không
        if self.image is None:
            final_path = image_path if image_path else default_image_path
            raise FileNotFoundError(
                f"LỖI: OpenCV không thể đọc được file ảnh. Đường dẫn đã kiểm tra: {final_path}"
            )

        # 3. Đọc file labels (nếu có)
        labels_path = os.path.join(self.data_fd, "labels", filename + '.csv')
        try:
            self.df_withlabels = pd.read_csv(labels_path)
        except FileNotFoundError:
            # Đây là trường hợp bình thường đối với ảnh mới, không phải lỗi
    ...
    def graph_formation(self,  use_clean_df=False,export_graph = False):
        ...
        # NẾU DÙNG DATAFRAME SẠCH TỪ OCR (cho dự đoán real-time)
        if use_clean_df:
            df = self.df.copy()
            # Đảm bảo các cột tọa độ là kiểu số
            for col in ['xmin', 'ymin', 'xmax', 'ymax']:
                df[col] = pd.to_numeric(df[col], errors='coerce')
            df.dropna(subset=['xmin', 'ymin', 'xmax', 'ymax'], inplace=True)

        # NẾU DÙNG CSV THÔ (cho training từ file)
        else:
        ...
        if not self.df_withlabels.empty and 9 in self.df_withlabels.columns:
            # Lấy cột nhãn bằng chỉ số integer 9
            df['labels'] = self.df_withlabels[9]
        else:
            # Nếu không có file nhãn (trường hợp ảnh mới), gán nhãn mặc định là 'other'
            df['labels'] = 'other'
